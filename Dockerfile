...
RUN pip3 install --no-cache-dir thrift thriftpy2 && \
    pip3 install --no-cache-dir mongoengine  && \
    pip3 install --no-cache-dir jieba synonyms

EXPOSE 9094

WORKDIR /expression-question
COPY ./ /expression-question

ENTRYPOINT ["python3.6", "-u", "server.py"]